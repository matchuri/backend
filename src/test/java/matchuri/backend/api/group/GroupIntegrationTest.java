...
class GroupIntegrationTest {
    ...
    @Test
    @DisplayName("그룹 추천 준비 세션 취소는 OWNER가 PREPARING 세션을 CANCELED로 전환하고 조회 API에 일관되게 노출한다")
    void cancelGroupRecommendationCancelsPreparingSessionForOwnerAndReturnsCanceledInReads() throws Exception {
        ...
        GroupRecommendation canceledRecommendation = groupRecommendationRepository.findById(recommendation.getId())
                .orElseThrow();
        ...
        mockMvc.perform(get("/api/v1/groups/{groupId}", groupRoom.getId())
                ...
                .andExpect(jsonPath("$.data.recentlyRecommendation.memberVotes.length()").value(0));
    ...
    @Test
    @DisplayName("그룹 추천 준비 세션 취소는 PREPARING 상태가 아니면 상태 충돌로 실패한다")
    void cancelGroupRecommendationFailsWhenRecommendationIsNotPreparing() throws Exception {
        ...
        finalizedRecommendation.finalizeWith(candidate, LocalDateTime.now().minusMinutes(5));
