...
class GroupIntegrationTest {
    ...
    @Test
    @DisplayName("그룹 추천 준비 세션 취소는 OWNER가 PREPARING 세션을 CANCELED로 종료하고 조회 응답에 반영한다")
    ...
    @Test
    @DisplayName("그룹 추천 준비 세션 취소는 OPEN, FINALIZED, EXPIRED 세션이면 상태 충돌로 거절한다")
    void cancelGroupRecommendationFailsWhenRecommendationIsNotPreparing() throws Exception {
        Member owner = saveMember("cancel-conflict-owner", "취소충돌방장");

        GroupRoom openGroup = saveGroupOwnedBy(owner, "취소 OPEN 그룹");
        ...
        GroupRoom finalizedGroup = saveGroupOwnedBy(owner, "취소 FINALIZED 그룹");
        ...
        GroupRoom expiredGroup = saveGroupOwnedBy(owner, "취소 EXPIRED 그룹");
        ...
        mockMvc.perform(patch("/api/v1/groups/{groupId}/recommendations/{sessionId}/cancel",
                        openGroup.getId(),
                        openRecommendation.getId())
                        .header(HttpHeaders.AUTHORIZATION, bearer(accessToken(owner))))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.error.code").value("GROUP_RECOMMENDATION_CANCEL_CONFLICT"));

        mockMvc.perform(patch("/api/v1/groups/{groupId}/recommendations/{sessionId}/cancel",
                        finalizedGroup.getId(),
                        finalizedRecommendation.getId())
                        .header(HttpHeaders.AUTHORIZATION, bearer(accessToken(owner))))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.error.code").value("GROUP_RECOMMENDATION_CANCEL_CONFLICT"));
