...
public interface GroupApi {
    ...
    @Operation(
            ...
            description = """
                    ...
                    - source 그룹 추천은 해당 그룹에 속하고 `PREPARING` 상태여야 합니다.
                    - `OPEN`, `FINALIZED`, `EXPIRED` 등 이미 준비 단계를 벗어난 세션은 상태 충돌로 거절합니다.
                    - 취소 후 그룹 추천 상태는 `CANCELED`가 되고 `endedAt`을 기록합니다.
                    - 취소된 세션은 그룹 상세의 최신 추천 상태, 추천 상세, 추천 목록에서 `CANCELED`로 조회됩니다.
                    """
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200",
                    description = "준비 세션 취소 성공",
                    content = @Content(
                            mediaType = "application/json",
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    ...
                    content = @Content(
                            ...
                            examples = @ExampleObject(
                                    name = "conflict",
                                    value = GroupApiExamples.CANCEL_RECOMMENDATION_CONFLICT
                            )
                    )
            )
    })
