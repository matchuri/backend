...
public interface GroupApi {
    ...
    @Operation(
            ...
            description = """
                    ...
                    - 대상 세션은 해당 그룹에 속한 `PREPARING` 상태여야 합니다.
                    - 이미 `OPEN`, `FINALIZED`, `EXPIRED` 등 `PREPARING`이 아닌 상태이면 상태 충돌로 처리합니다.
                    - 취소 후 그룹 추천 상태는 `CANCELED`가 되며 종료 시각을 저장합니다.
                    - 취소된 세션은 그룹 상세, 추천 상세, 추천 목록에서 `CANCELED` 상태로 조회됩니다.
                    """
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
